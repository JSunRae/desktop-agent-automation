...
- **Usage**: Override to use a different prompt file. This path is also used as the *root* for auto-discovered per-repo prompt feeds.
...
2. **Auto-generated layout**: Otherwise, look for `<FINISHED_PANEL_PROMPT_PATH parent>/<RepoName>/latest.txt` and use it *if it exists*.
...
```json
[
  {"name": "repo1", "docs_dirs": ["/path/to/repo1/docs", "/path/to/repo1/tasks"]},
