"""
Focus the RUNNING panel's text input and read content via clipboard
(This is the panel with Cancel button where you typed NEW TEXT AGAIN)
"""
...
def find_running_panel():
    """Find the running panel (has Cancel button)"""
    windows = get_vscode_windows()
    for win in windows:
        cancel_btn = has_button(win, "Cancel")
        if cancel_btn:
            return win, cancel_btn
    return None, None
...
print("=" * 70)
print("FOCUS AND READ RUNNING PANEL INPUT")
print("(Looking for the panel with Cancel button)")
print("=" * 70)

# Find the running panel
window, cancel_btn = find_running_panel()

if not window:
    print("\n❌ Could not find a RUNNING panel (no window with Cancel button found)")
else:
    title = window.Name[:60] + "..." if len(window.Name) > 60 else window.Name
    print(f"\n✓ Found RUNNING panel: {title}")
    if cancel_btn:
        print(f"  Cancel button at: {cancel_btn.BoundingRectangle}")
